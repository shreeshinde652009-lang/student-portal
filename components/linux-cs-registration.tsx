...
export function LinuxCsRegistration() {
  ...
  useEffect(() => {
    ...
    createClient().auth.getUser().then(async ({ data }) => {
      ...
      const { data: application } = await supabase.from('applications').select('id, application_number, submitted_at, personal_data, academic_data').eq('user_id', data.user.id).eq('course_name', 'Linux CS').maybeSingle()
      if (application && editMode) {
        ...
        for (const document of docs ?? []) {
          ...
          if (signed?.signedUrl) documentUrls[document.document_type] = signed.signedUrl
        }
  ...
  if (submitted && isReregistration && !isEditing) return <ApplicationPreview form={form} application={submitted} photoUrl={uploadedPhotoUrl} documentUrls={uploadedDocumentUrls} onEdit={() => setIsEditing(true)} />
...
function ApplicationPreview({ form, application, photoUrl, documentUrls, onEdit }: { form: FormState; application: { number: string; submittedAt: string }; photoUrl: string; documentUrls: Record<string, string>; onEdit: () => void }) {
