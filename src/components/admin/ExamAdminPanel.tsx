...
type Question = { id?: string; question_number: number; prompt: string; options: string[]; correct_option: string; marks: number };

const emptyQuestion = (questionNumber = 1): Question => ({ question_number: questionNumber, prompt: '', options: ['', '', '', ''], correct_option: '', marks: 1 });
...
export function ExamAdminPanel() {
  ...
  const selectExam = async (exam: Exam) => {
    ...
    const { data, error } = await client.from('exam_questions').select('id,question_number,prompt,options,correct_option,marks').eq('exam_id', exam.id).order('question_number');
  ...
  const saveExam = async (publish?: boolean) => {
    setNotice('');
    const client = createClient();
    const validQuestions = questions.filter((question) => question.prompt.trim() && question.options.filter((option) => option.trim()).length >= 2 && question.correct_option.trim());
    ...
    for (let index = 0; index < validQuestions.length; index += 1) {
      ...
      const questionPayload = { exam_id: exam.id, question_number: index + 1, prompt: question.prompt.trim(), options: question.options.filter((option) => option.trim()), correct_option: question.correct_option.trim(), marks: Math.max(1, Number(question.marks) || 1) };
    ...
    const { data: savedQuestions, error: reloadError } = await client.from('exam_questions').select('id,question_number,prompt,options,correct_option,marks').eq('exam_id', exam.id).order('question_number');
