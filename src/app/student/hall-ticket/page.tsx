...
export default function HallTicketPage() {
  ...
  useEffect(() => { const load = async () => { const supabase = createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) { router.replace('/student/login'); return; } const { data: settingRows } = await supabase.from('module_settings').select('key, value').eq('key', 'hall_ticket_enabled').maybeSingle(); if (parseSettingValue(settingRows?.value) !== true) { setState('unavailable'); return; } const { data } = await supabase.from('applications').select('*').eq('user_id', user.id).maybeSingle(); if (!data) { router.replace('/student/dashboard'); return; } setApplication({ ...data.personal_data, ...data.academic_data, applicationNumber: data.application_number }); setState('ready'); }; void load(); }, [router]);
