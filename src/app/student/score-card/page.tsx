...
export default function ScoreCardPage() {
  ...
  useEffect(() => { const load = async () => { const supabase = createClient(); const { data: { user } } = await supabase.auth.getUser(); if (!user) { router.replace('/student/login'); return; } const { data: settingRows } = await supabase.from('module_settings').select('key, value').in('key', ['score_card_enabled', 'score_card_published']); const settings = Object.fromEntries((settingRows ?? []).map((row) => [row.key, parseSettingValue(row.value)])); if (settings.score_card_enabled !== true) { setState('unavailable'); return; } if (settings.score_card_published !== true) { setState('not-published'); return; } const { data } = await supabase.from('applications').select('*').eq('user_id', user.id).maybeSingle(); if (!data) { router.replace('/student/dashboard'); return; } const { data: scoreCard } = await supabase.from('score_cards').select('*').eq('application_id', data.id).eq('status', 'published').maybeSingle(); if (!scoreCard) { setState('not-published'); return; } setApplication({ ...data.personal_data, ...data.academic_data, ...scoreCard, fullName: String(data.personal_data?.fullName || data.personal_data?.full_name || ''), applicationNumber: data.application_number }); setState('ready'); }; void load(); }, [router]);
