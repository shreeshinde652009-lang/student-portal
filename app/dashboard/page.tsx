...
const cards = [
  ['Linux CS Registration', 'Register Now', FileText],
...
export default function DashboardPage() {
  ...
  const [hasLinuxRegistration, setHasLinuxRegistration] = useState(false)
  ...
  useEffect(() => {
    ...
    createClient().auth.getUser().then(async ({ data }) => {
      ...
      setUser(data.user)
      const { data: application } = await createClient().from('applications').select('id').eq('user_id', data.user.id).eq('course_name', 'Linux CS').maybeSingle()
      if (active) setHasLinuxRegistration(Boolean(application))
    })
    return () => { active = false }
  }, [])
  async function signOut() { await createClient().auth.signOut(); window.location.href = '/' }
  function openCard(title: string) { if (title === 'Profile') setProfileOpen(true); else if (title === 'Linux CS Registration' || title === 'Re-registration') window.location.href = '/dashboard/linux-cs-registration?mode=edit'; else setActiveCard(title) }
